...
## 2026-09-07 — 번역 품질 패스 (RP-2 / CO-3 / BM-2)

인식(OCR)을 고친 뒤 남은 최대 결함: **정확히 읽은 문장을 번역기가 망친다.**

### 실태 (고치기 전에 잰 것)

앱의 진짜 경로(`perform_ocr_with_boxes` → `assign_region_languages` → `batch_translate`
→ `display_gate_reject`)로 태우고, 모델 자체는 영어 **1,940문장** + OCR 오독 주입 **600문장**
코퍼스로 따로 쟀다(시료 영어 문단이 70개뿐이라 0.4%급 사건의 빈도를 못 낸다).

| 유형 | 깨끗한 입력 1,940문장 | OCR 오독 600문장 | 판정 |
|---|---|---|---|
| **반복 붕괴** | **8건 = 0.41%** | 0건 | greedy 탐색 실패 |
| 문장 누락 | 0건 (189줄 문단 코퍼스에서도 0) | — | SE-1이 이미 막고 있다 |
| 길이 붕괴 | 1건(`Measurements`→`측정` = **정상 번역**) | — | 대리 지표의 거짓 경보 |
| 미번역 잔류 | 11건 — 전부 `user1:` 채팅 줄 | 다수(진짜) | **게이트 오탐**(CO-3) |

사용자가 본 `"그 때, 그 때, 한 때의 그 시절..."` 은 **문장 누락이 아니라 첫 문장의 반복 붕괴**였다.
원문 3문장은 전부 번역됐고, 1번 문장의 번역문이 통째로 딴 소리였을 뿐이다.

### 원인 (후보를 하나씩 실측 배제)

| 후보 | 판정 |
|---|---|
| SE-1 문장 분할 누락 | **기각** — 조각 3/3 번역됨. 문단을 통째로 넣으면 오히려 정상 번역 |
| PM-1 병합으로 앞 문장 유실 | **기각** — 누락 0/2129. 종결부호 뒤 공백이 없는 입력도 누락 0 |
| `no_repeat_ngram_size=4` | **유지가 맞다** — 0이면 `"그 때," ×16` 무한 루프, 2·3이면 반복만 줄고 뜻은 여전히 틀림 |
| **greedy(`num_beams=1`)** | **원인 확정** — 같은 문장을 `num_beams=4`로 뽑으면 정상. 원문에서 한 글자만 달라져도 붕괴가 사라진다 |

### 고친 것

| 코드 | 무엇 | 효과 |
|---|---|---|
| **RP-2** | 붕괴 징후가 있는 **조각만** `num_beams=4` 재시도, 더 나아졌을 때만 채택 | 붕괴 8건 중 **6건 정상 교체 · 악화 0건**. 초과반복 max +0.333 → **+0.100** |
| **CO-3** | `carryover_words` 가 숫자 끝에 붙은 식별자(`user1`,`mp3`,`PyQt5`)를 세지 않게. 글자 사이 숫자(`cre2ted`)는 계속 센다 | 채팅 게이트 오탐 **11/12줄 → 0/12줄** |
| **BM-2** | 채점판 기준 2에 반복 붕괴·문장 누락 **정확 측정** 추가 | 확정 오역 0건 → **1건**(원래 있던 결함이 이제 보인다) |

전면 `num_beams=4`는 **채택 불가**로 판정했다 — 배치 12문장 p50 311ms → 553ms(**+78%**)라
기준 4(p50 한도 1.5s, 현재 1.4~1.5s)를 깬다.

### 속도 (기준 4, 채점판 3회씩)

| | before | after |
|---|---|---|
| 전체 p50 | 1.45 / 1.43 / 1.38 s | 1.46 / 1.46 / 1.44 s |
| 전체 p95 | 2.52 / 2.23 / 2.48 s | **2.97 / 2.90 / 2.72 s** |
| 채팅 p50 | 1.80 / 1.80 / 1.86 s | 2.00 / 2.04 / 1.88 s |
| 번역 1회 p50 | 241 ms | 241 ms (변화 없음) |

재시도 1회 = **약 190ms**(빔 수·`max_new_tokens` 를 줄여도 안 준다 — 비용은 "generate가 한 번 더
도는 것"이다). 코퍼스 전체 시간은 48.5s → 48.0s로 변화가 없는데, 채점판 채팅 시나리오만
**QU-1 큐잉 누적** 구간이라 한 번의 190ms가 이후 표본 전부에 남는다. 통과하지만 마진이 얇다.

### 채점판

1·3·4·5 통과 / **2 실패**(반복 붕괴 1건 — 모델 한계, 아래) / 6 실패(`dist/` 재빌드 대기).
테스트 **65 → 70** (반복 붕괴 검출 / 정상 문장 오검출 없음 / 붕괴 조각만 빔 재시도 /
정상 프레임은 재시도 안 함 / CO-3 식별자).

### 모델 한계로 남는 것 (다음 판단 재료)

- `"Council meeting minutes" → "회의록 회의록"` — `num_beams=4`로도 같은 출력. 탐색 실패가
  아니라 모델이 그렇게 번역한다. **기준 2가 실패로 잡고 있는 유일한 건**이다.
- `"collected notes, page 41" → "리뷰, Page 41"`(뜻 훼손 + 미번역 잔류), beams=4는
  `"참고문헌 41 page"` — 역시 틀린다. `"Tide tables for August" → "8 월의 표"`(tables 유실).
  **짧은 명사구 라벨이 이 모델의 약점**이다(문장 쌍으로 학습된 모델).
- `"If omitted or None, the entire source is used." → "생략하거나 생략하지 않으면..."`(부정 뒤집힘).
  beams=4도 동일. 반복이 없어 어떤 기계 지표로도 못 잡는다.
- 즉 남은 번역 결함은 **디코딩이 아니라 모델 능력**이다. 이 이상은 B3(모델 교체)의 영역이다.
