...
## 2026-09-07 — 프라이버시 패스 (PV-1 / PV-2 / PV-3 / SB-2)

"전부 로컬"을 강점으로 내세우면서 **사용자 데이터 취급이 그 약속을 못 지키고 있었다.**
상세 원인·임계 근거는 `errors.md` 네 항목.

### 고치기 전에 잰 것

| | 실측 |
|---|---|
| 영구 캐시가 이 PC에 쌓아 둔 것 | **2,574항목 / 265KB** (지우는 UI·만료·끄는 옵션 **전부 없음**) |
| 캐시 축출 | LRU 아님 — 상한(4096)에 닿으면 **dict 앞쪽 절반을 무작정 삭제** |
| 암호화 범위 | DPAPI `CurrentUser` = **같은 윈도우 계정의 아무 프로세스나 복호화** |
| 로그 | `[INFO] DG-1 숨김: ... "원문 60자"` — 화면 글자가 콘솔·로그 파일에 남음 |
| 전역 키 후킹 | `keyboard`(모든 키 입력 가로챔)를 **기본 활성 + 최상단 import** |
| 민감 창 가드(SB-1) | 32개 실사용 시료에서 **놓침 25 = 78%** |
| 문서 | README/VISION이 **"번역 데이터 비저장 · 메모리 캐시만"** 이라고 적고 있었다 |

### 고친 것

| 코드 | 무엇 | 결과 |
|---|---|---|
| **PV-1** | 영구 캐시 **기본 끔** + 트레이 "번역 기록 삭제"(즉시·확인 피드백) + 트레이 "번역 기록을 디스크에 저장" 토글(QSettings 유지, **끄면 파일까지 삭제**) + **7일 만료** + **LRU 축출** | 기본 상태에서 디스크에 아무것도 안 남는다. 옛 형식 항목은 나이를 몰라 로드에서 버린다 |
| **PV-2** | `log_text()` — 로그의 원문을 **길이+sha1 8자리**로. 사유는 그대로 남긴다(CO-2). `COCKTAIL_LOG_TEXT=1` 이면 원문 | `[INFO] RP-2 반복 붕괴 재번역: [84자 #f8de909e]` |
| **PV-3** | 전역 단축키 **기본 끔** + 트레이 토글 + **라이브러리 import 자체를 켤 때로** 지연 | 끈 상태에서 `keyboard` 가 `sys.modules` 에 없다. 등록/해제 코드는 그대로 |
| **SB-2** | 민감 창 가드 미검출 78% **측정·기록**(고치는 건 별건) | 놓치는 부류를 errors.md에 열거 + README/VISION에 한계 명시 |

### 속도 영향 (기준 4)

**없다.** 채점판 기준 4는 애초에 영구 캐시를 끈 상태로 재기 때문이다(`bench/score.py`가
명시적으로 끈다). 실측 p50 **1.42s** / p95 **2.82s** — 직전 값(1.42 / 2.86)과 같은 범위.

영구 캐시가 실제로 버는 시간도 따로 쟀다(모델 로드 후, 22줄 화면, 3회 p50):

| | 22줄 화면 | 줄당 |
|---|---|---|
| 캐시 끔 (매번 새로 번역) | 162 ms | 7.4 ms |
| 캐시 켬 (지난 실행 문장 재방문) | 0.1 ms | 0.0 ms |

즉 영구 캐시가 버는 것은 **재방문 화면당 한 번 162 ms**뿐이다. 같은 세션의 반복은
메모리 LRU가 이미 전부 잡으므로 영구 캐시가 없어도 0.1 ms다. 이 이득에
"화면에서 본 모든 문장의 영구 기록"을 기본으로 켜 둘 이유가 없다고 판단했다.

### 채점판 (2026-09-07, 이 패스 후)

| # | 판정 | 측정값 |
|---|---|---|
| 1 | 통과 | 14px 이하 CER 최대 **2.8%** |
| 2 | 실패 | 오탐 0/79 = 0.0% · 확정 오역 **1건**(`"Council meeting minutes"→"회의록 회의록"` = 모델 한계, 의도적 보존) |
| 3 | 통과 | doc 0 · fullhd 0 · multi_context 0 |
| 4 | 통과 | **p50 1.42s / p95 2.82s** · 누락 0 · 오염 아니오 |
| 5 | 통과 | 이탈 0 · 잘림 0 · 트레이 **13개**(10→13) · 단축키 **2개** · 첫 실행 안내 O |
| 6 | 실패 | 배포물이 소스보다 낡음(재빌드 대기, 이번에 안 함) |

테스트 **70 → 76**(신규 6: 꺼진 상태에서 디스크 미기록 / 삭제·끄기가 파일 삭제 /
만료 / LRU 축출 / 로그 원문 미누출 / 프라이버시 기본값 OFF + 그 상태에서 앱 정상).

### 안 고친 것

- **SB-1 민감 창 가드**(SB-2) — 별건. 방향은 ①창 클래스 화이트리스트
  (`Credential Dialog Xaml Host`) ②브라우저 URL을 UIA로 읽어 도메인 판정 ③출력측 패턴 가드.
- **DPAPI 엔트로피** — `CryptProtectData`에 앱 고유 엔트로피를 주면 남의 프로세스가
  그냥은 못 푼다. 그러나 그 엔트로피가 exe 안에 있으므로 **결정된 공격자에겐 무의미**하고,
  "암호화를 강화했다"는 잘못된 안심을 준다. 기본을 끄는 쪽이 정직하다고 판단해 안 넣었다.
- **로그 파일 회전/삭제** — 앱은 파일 로그를 직접 쓰지 않는다(`debug_run.bat`이 리다이렉트할 뿐).
  PV-2로 원문이 안 나가므로 남는 것은 진단 문자열뿐이다.
