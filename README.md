...
- ✅ 트레이 상주, 민감 창 자동 일시정지, 시작 프로그램 등록
- ✅ 프라이버시 기본값(PV-1/PV-3) — 번역 기록 디스크 저장·전역 키 후킹 **둘 다 기본 꺼짐**(트레이에서 켜기).
  켜도 7일 만료 + LRU 축출 + "번역 기록 삭제" 한 번으로 파일까지 삭제
...
요약: **로컬 추론(화면·번역 데이터 외부 전송 없음) · 디스크 기록 기본 없음 · 민감 영역 자동 OFF ·
오픈 코어 · 즉시 토글**.

외부 통신은 **최초 1회 모델 다운로드(Hugging Face)** 뿐입니다. 그 이후 실행에서는
번역·OCR 모두 로컬에서만 수행되며 화면 내용은 네트워크로 나가지 않습니다.

### 이 앱이 이 PC에 남기는 것 (PV-1, 2026-09-07)

| | 기본값 | 어디에 | 지우는 법 |
|---|---|---|---|
| 번역 결과 메모리 캐시 | 켬 | RAM(1024항목) | 앱을 끄면 사라짐 · 트레이 "번역 기록 삭제" |
| **번역 기록 디스크 저장** | **끔** | `~/.cocktail/translation_cache.bin` (DPAPI 암호화) | 트레이 "번역 기록 삭제" · 옵션을 끄면 파일까지 삭제 · 켜 둬도 **7일 뒤 자동 만료** |
| 사용자 설정(언어·모드·영역) | 켬 | 레지스트리 `HKCU\Software\Cocktail` | 설정만 저장. 화면 내용은 안 들어감 |
| 로그(`print`) | 켬 | 콘솔 | 화면 원문은 안 찍음 — 길이+해시로만 (`COCKTAIL_LOG_TEXT=1` 이면 원문, 개발용) |

**"번역 기록 디스크 저장"이 기본으로 꺼져 있는 이유**: 켜면 지나간 화면의 번역이 이 PC에
쌓입니다. DPAPI 암호화는 *다른 사용자·다른 PC*로부터 지켜 줄 뿐, **같은 윈도우 계정으로
도는 다른 프로그램은 그대로 복호화할 수 있습니다.** 켜면 지난 실행에서 본 화면을 다시
볼 때 22줄 기준 0.16초가 빨라집니다 — 그 정도 이득에 영구 기록을 기본으로 두지 않았습니다.

**전역 단축키(Ctrl+Shift+Q/A)도 기본 꺼짐**입니다. 이 기능이 쓰는 `keyboard` 라이브러리는
모든 키 입력을 가로채는 후킹이라(백신이 경고할 수 있습니다) 켤 때만 불러옵니다.
꺼진 상태에서도 트레이 메뉴와 창 단축키(Ctrl+Shift+H, Ctrl+Shift+P)로 모든 기능을 씁니다.
둘 다 트레이 메뉴에서 켤 수 있습니다.

⚠ **민감 창 자동 OFF는 창 제목 키워드 기반이라 완전하지 않습니다** — 영문 뱅킹·결제
사이트·비밀번호 관리자·OTP 앱 등은 놓칩니다(실측 미검출 78%, errors.md SB-2).
민감한 화면에서는 번역을 직접 정지시키세요.
